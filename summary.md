...
日期: 2026-09-04 版本: Alpha - Battle Menu 引擎: Unity (2022.3.61t9 / Tuanjie 1.6.8)
...
2. 最近进展 (Recent Progress, snapshot 2026-09-04)
*   **房间与 AI v12 重构（完成，2026-09-04）**:
    *   删除自动 AI 补位，房主在四个权威席位上手动添加、更新和移除永久 AI；房主身份使用稳定 `playerId`，断线保留 120 秒并由服务端广播转移结果。
    *   永久 AI 构筑公开、真人构筑私有；真人主动退出或超时转出的永久 AI 沿用锁定构筑并默认新手难度。
    *   新手与标准难度共用服务端合法性边界，标准策略评估普通牌型、七对、十三幺、合法起胡和改良牌种，20ms 软截止后返回已评估最佳候选。
    *   房间页完成现代国风 2x2 席位台与快速配置；高级 AI 构筑复用玩家 `DeckEditorToolkit`，通过上下文切换隔离房间草稿与 Profile 持久化。
*   **对战菜单与退出流程（完成，2026-09-04）**:
    *   原型已确认；独立菜单提供继续对战、共享音频设置、二次确认退出及 Esc 分层返回，打开菜单不暂停联机对局。
    *   统一可等待离房流程，最多等待 5 秒发送完成；退出即清票据并阻断旧房间消息，场景导航串行化且旧恢复路由不能重新激活对战。
    *   独立手牌/动作输入限制保护菜单遮罩及关闭当帧，不恢复已经过期的决策。加载、重连和结算面板隐藏时整个文档根节点退出输入层。
    *   完整 `NetworkRegression` 已通过；MCP 已完成 Refresh、编译、独立菜单场景接线与三页渲染检查，真实本地房间退出并再次开局成功，连接与音频实例复用正常。
    *   用户已完成真实键鼠、跨面板输入、不同窗口比例与听音验收，结果无问题；验证记录见 `docs/battle_menu_verification.md`。
    *   `AGENTS.md` 已记录项目 MCP 持续授权，无需逐次确认。
...
*   **模式起始分与击飞终局完成 (2026-09-02)**:
    ...
    *   `SessionEnd` 下发最终分数、完成局数、终局原因和耗尽席位。客户端只接受该终局权威，保存只读姓名/结算状态并清理活动房间与重连票据。
...
*   **天赋玩法垂直切片与联机架构完成 (2026-08-16)**:
    *   一人游玩和多人游玩统一进入在线 `Room`；当前由房主逐席配置永久 AI，`GameManager` 只协调权威网络投影、恢复与场景，不持有服务端、会话或天赋 runtime。
